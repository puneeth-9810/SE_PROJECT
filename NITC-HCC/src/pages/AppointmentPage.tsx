...
const AppointmentPage = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    if (!data.email || !data.email.includes('@nitc.ac.in')) {
      toast({
        title: "Invalid email",
    ...
    try {
      // Step 1: Send Patient Data to Backend
      const formattedDate = format(new Date(data.date), "yyyy-MM-dd");

      const response = await fetch("http://localhost:5000/a", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: data.name, email: data.email ,reason: data.reason ,date: formattedDate,}),
      });
      ...
      form.reset({ email: "", name: "", reason: "", date: undefined });
  
    } catch (error) {
      console.error("Error:", error);
      toast({
        title: "Error",
        description: "Failed to book appointment. Try again.",
        variant: "destructive",
      });
    }
  };
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="Your mail" {...field} className="rounded-md shadow-sm" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
